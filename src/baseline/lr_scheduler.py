...
def get_scheduler(optimizer, max_iters, iters_per_epoch):
    mode = "poly"
    warm_up_iters = iters_per_epoch * 10
    if mode == 'poly':
        return WarmupPolyLR(optimizer, max_iters=max_iters, power= 0/9,
                            warmup_factor= 1.0 / 3 , warmup_iters=warm_up_iters,
                            warmup_method= 'linear')
    elif mode == 'cosine':
        return WarmupCosineLR(optimizer, max_iters=max_iters, warmup_factor=1.0 / 3,
                              warmup_iters=warm_up_iters, warmup_method=  'linear')
    elif mode == 'step':
        milestones = [x * iters_per_epoch for x in [10, 20]]
        return WarmupMultiStepLR(optimizer, milestones=milestones, gamma= 0.1,
                                 warmup_factor=1.0 / 3, warmup_iters=warm_up_iters,
                                 warmup_method= 'linear')
    else:
